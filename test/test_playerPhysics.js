...
assert(player.width === 0.8, 'Player width should be 0.8 blocks');
...
player2.update(0.1, { forward: false, backward: false, left: false, right: false, jump: false }, null);
...
player2.update(0.5, { forward: false, backward: false, left: false, right: false, jump: false }, null);
assert(player2.position.y < startY, 'Player should fall when no ground');

// --- Test 7: Jump works when on ground ---
const player3 = new Player();
player3.onGround = true;
player3.update(0.1, { forward: false, backward: false, left: false, right: false, jump: true }, null);
...
player4.update(0.1, { forward: false, backward: false, left: false, right: false, jump: true }, null);
...
player5.yaw = 0; // Facing +Z (forward)
player5.update(0.1, { forward: true, backward: false, left: false, right: false, jump: false }, null);
...
player6.yaw = 0;
player6.update(0.1, { forward: true, backward: false, left: false, right: false, jump: false }, null);
assert(player6.velocity.z > player.moveSpeed, 'Sprint should increase velocity beyond base move speed');
...
assert(player.position.y === 20, 'Default respawn Y should be 20');
assert(player.position.z === 0, 'Default respawn Z should be 0');

// --- Test 15: _isSolid correctly identifies solid blocks ---
assert(player._isSolid(BLOCK_TYPES.STONE) === true, 'Stone should be solid');
assert(player._isSolid(BLOCK_TYPES.DIRT) === true, 'Dirt should be solid');
assert(player._isSolid(BLOCK_TYPES.GRASS) === true, 'Grass should be solid');
assert(player._isSolid(BLOCK_TYPES.WOOD_LOG) === true, 'Wood log should be solid');

// --- Test 16: _isSolid correctly identifies non-solid blocks ---
assert(player._isSolid(BLOCK_TYPES.AIR) === false, 'Air should not be solid');
assert(player._isSolid(BLOCK_TYPES.WATER) === false, 'Water should not be solid');
assert(player._isSolid(BLOCK_TYPES.LEAVES) === false, 'Leaves should not be solid');

// --- Test 17: Diagonal movement normalization ---
const player7 = new Player();
player7.yaw = Math.PI / 4; // 45 degrees
player7.update(0.1, { forward: true, backward: false, left: true, right: false, jump: false }, null);
