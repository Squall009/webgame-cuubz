...
it('maxPlayerAndDisconnect', () => legacy(async () => {
// ─── Test Harness ──────────────────────────────────────────────
...
// D-20: this file used to bind the FIXED ports 18780 (matchmaking) and 18781+ (game
...
// `integration/multiplayerStress.test.js` already used.
//
...
const matchmaking = new Matchmaking({
  ...
  onHostRequest: (playerId, sessionName, worldSeed, mode) => {
    ...
    sessionHttp.on('error', (err) => {
      assert(false, `test/integration/maxPlayerAndDisconnect.test.js: game-session listener on ephemeral port 0 failed to bind (${err.code || err.message})`);
    });
...
const serverReady = new Promise((resolve, reject) => {
  matchmakingHttp.on('error', (err) => {
    const msg = `test/integration/maxPlayerAndDisconnect.test.js: matchmaking listener error on ephemeral port 0 (${err.code || err.message})`;
