...
it('multiplayerStress', () => legacy(async () => {
...
async function runTests() {
  ...
  // D-20: this file already bound port 0 — that half was the pattern the four other
  // socket suites were fixed to. The other half of the row is that neither listener here
  // had an 'error' handler, so a bind that failed anyway (a leftover process holding the
  // handle, an exhausted ephemeral range) surfaced as an unhandled EADDRINUSE naming
  // neither this file nor the port. This is `integration/server.test.js`'s shape, not a
  // second invention: reject before the bind completes, assert after it, so a late error
  // cannot be swallowed by an already-settled promise.
  let matchListening = false;
  await new Promise((resolve, reject) => {
    matchHttp.on('error', (err) => {
      const msg = `test/integration/multiplayerStress.test.js: matchmaking listener error on ephemeral port 0 (${err.code || err.message})`;
      if (matchListening) assert(false, msg);
      else reject(new Error(msg));
    });
    matchHttp.listen(0, () => { matchListening = true; resolve(); });
  });
  ...
  new Matchmaking({
    ...
    onHostRequest: (pid, name, seed, mode) => {
      ...
      // D-20: the game-session listener's half. This callback must return the port
      // synchronously, so the read stays synchronous — `listen()` binds the handle before
      // it returns when no hostname lookup is needed — and the error handler is the
      // synchronous `assert(false, …)` form `integration/server.test.js` uses in the same
      // position, because there is no promise here to reject.
      sh.on('error', (err) => {
        assert(false, `test/integration/multiplayerStress.test.js: game-session listener on ephemeral port 0 failed to bind (${err.code || err.message})`);
      });
