...
it('multiplayerSync', () => legacy(async () => {
// ─── Test Harness ──────────────────────────────────────────────
...
// D-20: this file used to bind the FIXED ports 18770 (matchmaking) and 18771+ (game
// sessions) with no 'error' listener on either, so an occupied port died on an
// unhandled EADDRINUSE naming neither this file nor the port. It now binds port 0 and
// reads the assigned port back off the server — the pattern
// `integration/multiplayerStress.test.js` already used. "Unique port range for this
// test" is no longer something a human has to keep true.
//
// `MATCHMAKING_PORT` is 0 until the `serverReady` listen callback below fires; every
// read of it is behind `await serverReady`.
...
const matchmaking = new Matchmaking({
  ...
  onHostRequest: (playerId, sessionName, worldSeed, mode) => {
    ...
    // does. This callback returns the port synchronously, which is why the read is the
    // synchronous one — `listen()` with no hostname binds the handle before it returns.
    sessionHttp.on('error', (err) => {
      assert(false, `test/integration/multiplayerSync.test.js: game-session listener on ephemeral port 0 failed to bind (${err.code || err.message})`);
    });
...
// D-20: ephemeral port, real port read back once listening, and a rejection — not an
// unhandled 'error' event — if the bind fails, so the failure names this file.
let mmListening = false;
const serverReady = new Promise((resolve, reject) => {
  matchmakingHttp.on('error', (err) => {
    const msg = `test/integration/multiplayerSync.test.js: matchmaking listener error on ephemeral port 0 (${err.code || err.message})`;
  ...
  matchmakingHttp.listen(0, '127.0.0.1', () => {
    mmListening = true;
