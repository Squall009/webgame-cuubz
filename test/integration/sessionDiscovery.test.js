...
it('sessionDiscovery', () => legacy(async () => {
// ─── Test Infrastructure ──────────────────────────────────────
...
//
// D-20: this file used to bind the FIXED ports 18790 (matchmaking) and 18791+ (game
...
//
// `MATCHMAKING_PORT` is 0 until `runTests()` has the listener up; every read of it —
// including `connectMatchmaking()` below — happens after that.
...
const matchmaking = new Matchmaking({
  ...
  onHostRequest: (playerId, sessionName, worldSeed, mode) => {
    ...
    sessionHttp.on('error', (err) => {
      assert(false, `test/integration/sessionDiscovery.test.js: game-session listener on ephemeral port 0 failed to bind (${err.code || err.message})`);
    });
...
async function runTests() {
  // Start servers. D-20: ephemeral port, and a rejection — not an unhandled 'error'
  // event — if the bind fails, so the failure names this file.
  let mmListening = false;
  await new Promise((resolve, reject) => {
    matchmakingHttp.on('error', (err) => {
      const msg = `test/integration/sessionDiscovery.test.js: matchmaking listener error on ephemeral port 0 (${err.code || err.message})`;
