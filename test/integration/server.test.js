...
// D-20: this file used to bind the FIXED ports 18765 (matchmaking) and 18766+ (game
// sessions) with no 'error' listener on either. Anything else already holding one of
// them — a leftover process, another checkout, a developer's own relay — killed the run
// with an unhandled EADDRINUSE that named neither this file nor the port. It now binds
// port 0 and reads the assigned port back off the server, which is the pattern
// `integration/multiplayerStress.test.js` already used; the complaint was that the five
// socket suites disagreed, so this is that file's shape, not a second invention.
//
// `MATCHMAKING_PORT` is 0 until `runTests()` has the listener up. Every read of it is
// inside `runTests()`, after the listen resolves.
let MATCHMAKING_PORT = 0;
...
const matchmaking = new Matchmaking({
  ...
  onHostRequest: (playerId, sessionName, worldSeed, mode) => {
    ...
    // D-20: `listen(0)` then read the port back, exactly as multiplayerStress.test.js
    // does. This callback must return the port synchronously, so the read is the
    // synchronous one — `listen()` binds the handle before it returns when no hostname
    // lookup is needed, which is why that file's version works.
    sessionHttp.on('error', (err) => {
      assert(false, `test/integration/server.test.js: game-session listener on ephemeral port 0 failed to bind (${err.code || err.message})`);
    });
    sessionHttp.listen(0);
    const sessionPort = sessionHttp.address().port;
...
async function runTests() {
  // Start matchmaking server. D-20: ephemeral port, and a rejection — not an unhandled
  // 'error' event — if the bind fails, so the failure names this file.
  let mmListening = false;
  await new Promise((resolve, reject) => {
    matchmakingHttp.on('error', (err) => {
      const msg = `test/integration/server.test.js: matchmaking listener error on ephemeral port 0 (${err.code || err.message})`;
      // Before the bind completes the failure is the bind; after it, a late error must
      // still fail the suite rather than be swallowed by an already-settled promise.
      if (mmListening) assert(false, msg);
      else reject(new Error(msg));
    });
    matchmakingHttp.listen(0, () => { mmListening = true; resolve(); });
  });
  MATCHMAKING_PORT = matchmakingHttp.address().port;
